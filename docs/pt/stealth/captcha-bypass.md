...
- **Consistência do fingerprint.** A identidade que o seu navegador apresenta tem que concordar consigo mesma e com o seu IP. Uma falha comum é uma incompatibilidade de versão do Chrome: se você combinar isto com a [Injeção de fingerprint](fingerprint-injection.md), a versão anunciada pelo perfil tem que corresponder ao binário real, ou o Turnstile fica preso em "Just a moment...". Alinhe-a a `await browser.get_version()`.
- **Headful vs headless.** O headless vaza sinais de renderização que reduzem a pontuação de confiança. Prefira headful para o Turnstile, ou neutralize os sinais de headless antes (veja [Injeção de fingerprint](fingerprint-injection.md)).
