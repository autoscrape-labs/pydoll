...
- **Headful vs headless.** Headless emits a weaker display signal that can lower the trust score, but it is not a wall. With a coherent fingerprint and a locale matched to the IP, headless clears Turnstile on a decent IP. On a marginal IP, prefer headful, or headful under a virtual framebuffer (Xvfb) on a server, so the display signal stops counting against you.
