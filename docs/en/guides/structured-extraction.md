...
- [Element finding](element-finding.md): the `find()` and `query()` calls extraction is built on.
- [Selectors: CSS and XPath](../basics/selectors.md): write the selectors your fields use.
- [DOM traversal](dom-traversal.md): when a page needs manual navigation instead of a model.
