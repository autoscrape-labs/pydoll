...
- **Headful 与 headless。** Headless 会发出较弱的显示信号，可能降低信任分，但这并不是一堵无法逾越的墙。只要 fingerprint 自洽，并且 locale 与 IP 相匹配，在一个不错的 IP 上 headless 也能通过 Turnstile。在勉强及格的 IP 上，优先使用 headful，或者在服务器上用虚拟帧缓冲（Xvfb）跑 headful，让显示信号不再对你不利。
