...
class ConnectionHandler:
    ...
    def _use_ws_address(self, ws_address: str):
        """Using the ws address, update host, port and use_secure to reflect it."""
        ...
        if parsed_url.port is None:
            self._connection_port = 443 if self._use_secure else 80
        else:
            self._connection_port = parsed_url.port

    def _get_ws_scheme(self):
        """Get the WebSocket scheme to use."""
        if self._use_secure:
            return 'wss'
        return 'ws'

    def _get_host_formatted(self):
        """Formats the host on the base of ipv4 or ipv6 in case of using IP as host."""
